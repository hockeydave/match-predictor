...
def build_model(results: List[Result]) -> Tuple[AdaBoostClassifier, OneHotEncoder]:
...
def train_adaboost_predictor(results: List[Result]) -> Predictor:
    model, team_encoding = build_model(results)

    return AdaBoostPredictor(model, team_encoding)
